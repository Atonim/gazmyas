...
const Manager = {
    ...
    execute(taken, ta, pl) {
        ...
        if (ta.state.next) {
            ...
            if (ta.nodes[ta.current]) return this.nextState(taken, ta, pl);
            if (ta.edges[ta.current]) return this.nextEdge(taken, ta, pl);
        } // Переход не нужен
        if (ta.nodes[ta.current]) return this.executeState(taken, ta, pl);
        if (ta.edges[ta.current]) return this.executeEdge(taken, ta, pl);
        throw `Unexpected state: ${ta.current}`;
    },
    nextState(taken, ta, pl) {
        ...
        for (let name of node.e) {
            ...
            for (let e of edge) {
                ...
                return this.execute(taken, ta, pl); // Рекурсивный вызов
            }
        }
    },
    nextEdge(taken, ta, pl) {

...
module.exports = {
  ...
  solveby2(d1, d2, x1, y1, x2, y2, e1, e2, x_bound, y_bound) {
    ...
    if (x1 == x2) {
      y =
        (this.squares_diff(y2, y1) + this.squares_diff(d1, d2)) /
        (2 * (y2 - y1));
      ...
      x =
        (this.squares_diff(x2, x1) + this.squares_diff(d1, d2)) /
        (2 * (x2 - x1));
      ...
      let beta =
        (this.squares_diff(y2, y1) +
          this.squares_diff(x2, x1) +
          this.squares_diff(d1, d2)) /
        (2 * (x2 - x1));
  ...
  get_object_coords(d1, da, x, y, x1, y1, a1, aa, eo) {
    let d_a1 =
      d1 * d1 +
      da * da -
      2 * d1 * da * Math.cos((Math.abs(a1 - aa) * Math.PI) / 180);
  ...
  see_object(obj_name, see_data) {
    ...
    for (const obj of see_data) {
      ...
      if (
        ...
        (obj_name === "p" &&
          cur_obj_name.includes(obj_name) &&
          !cur_obj_name.includes("f") &&
          !cur_obj_name.includes("Combine"))
      ) {
        return obj["p"];
      }
    }
  ...
  seeTopFlags(p) {
    for (const obj of p) {
      ...
      let cur_obj_name = obj["cmd"]["p"].join("");
      if (cur_obj_name[0] === "f" && cur_obj_name.includes("t")) {
        return true;
      }
    }
    return false;
